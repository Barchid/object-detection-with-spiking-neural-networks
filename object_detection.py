...
def main():
    ...
    # Dataset
    parser.add_argument(
        "-dataset", default="gen1", type=str, help="dataset used {GEN1}"
    )
    parser.add_argument(
        "-path", default="PropheseeGEN1", type=str, help="path to dataset location"
    )
    ...
    parser.add_argument(
        "-num_workers", default=8, type=int, help="number of workers for dataloaders"
    )
    parser.add_argument(
        "-no_train",
        action="store_false",
    ...
    parser.add_argument(
        "-precision", default=16, type=int, help="whether to use AMP {16, 32, 64}"
    )
    parser.add_argument("-save_ckpt", action="store_true", help="saves checkpoints")
    parser.add_argument(
        "-comet_api", default=None, type=str, help="api key for Comet Logger"
    )
    ...
    parser.add_argument(
        ...
        default="vgg-11",
        type=str,
        help="model used {squeezenet-v, vgg-v, mobilenet-v, densenet-v}",
        dest="model",
    )
    parser.add_argument(
        "-no_bn", action="store_false", help="don't use BatchNorm2d", dest="bn"
    )
    parser.add_argument(
        "-pretrained_backbone",
        default=None,
    ...
    parser.add_argument(
        "-pretrained", default=None, type=str, help="path to pretrained model"
    )
    parser.add_argument(
        "-extras",
        type=int,
    ...
    parser.add_argument(
        ...
        default=0.05,
        type=float,
        help="min ratio for priors' box generation",
    )
    parser.add_argument(
        "-max_ratio",
        default=0.80,
        type=float,
        help="max ratio for priors' box generation",
    )
    parser.add_argument(
        "-aspect_ratios",
        default=[[2], [2, 3], [2, 3], [2, 3], [2], [2]],
        type=int,
    ...
    trainer = pl.Trainer(
        ...
        logger=pl.loggers.TensorBoardLogger(
            "experiments/detections", name=f"{args.model}"
        ),
    )
    ...
    if args.train:
        ...
        repp = open("report_detection.txt", "a")
        repp.write(f"{ckpt_callback.best_model_score}\n")
    if args.test:
        test_dataset = dataset(args, mode="test")
        test_dataloader = DataLoader(
            test_dataset,
